...
const PRIMARY_NAV = '<nav class="nav nav--primary" aria-label="Main Menu">';
...
// The compact variant mounts on skl_id_search, and the widget renders it
// without the hero-input class the body bar gets, so the two need separate
// rules. Only one of the two mounts may exist per page, which is why this is
// applied to the pages that do not already carry the hero bar.
//
...
function addNavSearch(html) {
  const navStart = html.indexOf(PRIMARY_NAV);
  ...
  const mount = `<div id="skl_id_search" chat_id="${SHARE_ID}" isHero="false"></div>`;
  // .nav--primary is display:block above 1280px, so the mount would drop onto
  // its own line under the menu. It becomes a flex row only at that breakpoint:
  // below it the same element is display:none until the hamburger expands it,
  // and forcing flex there would pin the mobile menu open.
  //
  // The 15px bottom margin cancels the nav links' asymmetric 10px/25px padding,
  // which otherwise centres the bar 7.5px below the text baseline.
  const styles = `<style>
#skl_id_search{display:flex;align-items:center;min-height:40px}
...
@media(min-width:1280px){.nav--primary{display:flex!important;flex-wrap:wrap;justify-content:flex-end;align-items:center;column-gap:24px;width:100%}.nav--primary .nav__list{flex:0 0 auto}.nav--primary .nav__list>.nav__item>.nav__link{white-space:nowrap}#skl_id_search{flex:0 1 250px;min-width:200px;margin-bottom:15px}}
@media(max-width:1279px){#skl_id_search{width:100%;padding:0 20px 20px;box-sizing:border-box}}
</style>`;

  return (html.slice(0, navEnd) + mount + html.slice(navEnd)).replace(
    "</head>",
    `${styles}</head>`,
  );
}
